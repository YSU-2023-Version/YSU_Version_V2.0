...
double * AngleSolver::SolveAngle(vector<Point2f>& object_armor_points_,double p_y_recv[4])
{
 ...
 _yErr = _yErr-(CAM_SUPPORT_DIS+4.9*gra_t*gra_t)*0.001;
    ...
    p_y_err[0] = p_y_err[0] * 180 / CV_PI -1;
    p_y_err[1] = -p_y_err[1]*180/CV_PI + 4;

    double pre_time=+13;//+50激进，+15
...
//    out_mcu_data[3] = mcu_data.yaw_speed;
    //p_y_err[0]+=p_y_recv[1]*pre_time;

    int a=p_y_recv[3]*10;
    p_y_recv[3]=(double)a/10;
    p_y_err[1]+=3;
    if(p_y_err[1]>15)
        p_y_err[1]=0;
    ...
    std::cout << "                                  yaw=" << p_y_err[0] <<" ,"<<p_y_recv[2]+ p_y_err[0]-p_y_recv[3]*pre_time<< std::endl;
    std::cout << "pitch=" << p_y_err[1] << std::endl;
    cout<<"                                                     yaw speed: "<<p_y_recv[3]<<endl<<endl;
    //cout<<"                                                     yaw speed*time: "<<p_y_recv[3]*pre_time<<endl<<endl;
